...
public class MatchService {
    ...
    public Match createMatch(String team1Id, String team2Id, LocalDateTime date) {
        Match match = Match.builder()
                ...
                .team1Setup(Match.TeamSetup.builder().build())
                .team2Setup(Match.TeamSetup.builder().build())
                .officials(Match.MatchOfficials.builder().build())
                .timing(Match.MatchTiming.builder().build())
                .build();
    ...
    public Match startMatch(String matchId) {
        ...
        if (match.getTiming() == null) {
            match.setTiming(Match.MatchTiming.builder().build());
        }
        match.getTiming().setStartTime(LocalDateTime.now());
    ...
    public Match finishMatch(String matchId) {
        ...
        match.getTiming().setEndTime(LocalDateTime.now());
    ...
    /**
     * Add additional time to the match
     */
    public Match addAdditionalTime(String matchId, int additionalTime) {
        ...
        match.getTiming().setAdditionalTime(additionalTime);
        return matchRepository.save(match);
    }
    ...
    /**
     * Define the lineup for a team
     */
    public Match defineLineup(String matchId, String teamId, List<String> playerIds) {
        Match match = matchRepository.findById(matchId).orElseThrow(() -> new ResourceNotFoundException("Match not found with id " + matchId));
        Match.TeamSetup setup = getOrCreateTeamSetup(match, teamId);
        setup.setLineup(playerIds);
        return matchRepository.save(match);
    }

    /**
     * Define the substitutes for a team
     */
    public Match defineSubstitutes(String matchId, String teamId, List<String> playerIds) {
        Match match = matchRepository.findById(matchId).orElseThrow(() -> new ResourceNotFoundException("Match not found with id " + matchId));
        Match.TeamSetup setup = getOrCreateTeamSetup(match, teamId);
        setup.setSubstitutes(playerIds);
        return matchRepository.save(match);
    }

    /**
     * Define the tactical system for a team
     */
    public Match defineTacticalSystem(String matchId, String teamId, String tacticalSystem) {
        Match match = matchRepository.findById(matchId).orElseThrow(() -> new ResourceNotFoundException("Match not found with id " + matchId));
        Match.TeamSetup setup = getOrCreateTeamSetup(match, teamId);
        setup.setTacticalSystem(tacticalSystem);
        return matchRepository.save(match);
    }
    ...
    public Match assignCaptain(String matchId, String teamId, String playerId) {
        Match match = matchRepository.findById(matchId).orElseThrow(() -> new ResourceNotFoundException("Match not found with id " + matchId));
        Match.TeamSetup setup = getOrCreateTeamSetup(match, teamId);
        setup.setCaptainId(playerId);
        return matchRepository.save(match);
    }

    /**
     * Helper method to get or create team setup
     */
    private Match.TeamSetup getOrCreateTeamSetup(Match match, String teamId) {
        if (teamId.equals(match.getTeam1Id())) {
            if (match.getTeam1Setup() == null) {
                match.setTeam1Setup(Match.TeamSetup.builder().build());
            }
            return match.getTeam1Setup();
        } else if (teamId.equals(match.getTeam2Id())) {
            if (match.getTeam2Setup() == null) {
                match.setTeam2Setup(Match.TeamSetup.builder().build());
            }
            return match.getTeam2Setup();
        } else {
    ...
    /**
     * Assign the main referee
     */
    public Match assignMainReferee(String matchId, String name) {
        Match match = matchRepository.findById(matchId).orElseThrow(() -> new ResourceNotFoundException("Match not found with id " + matchId));
        if (match.getOfficials() == null) {
            match.setOfficials(Match.MatchOfficials.builder().build());
        }
        match.getOfficials().setMainReferee(name);
        return matchRepository.save(match);
    }
    ...
    public Match assignAssistantReferees(String matchId, List<String> names) {
        ...
        match.getOfficials().setAssistantReferees(names);
        return matchRepository.save(match);
    }

    /**
     * Assign the fourth official
     */
    public Match assignFourthOfficial(String matchId, String name) {
        ...
        match.getOfficials().setFourthOfficial(name);
        return matchRepository.save(match);
    }
    ...
    public Match assignVarReferees(String matchId, List<String> names) {
        ...
        match.getOfficials().setVarReferees(names);
        return matchRepository.save(match);
    }
    ...
    /**
     * Calculate the total duration of a match in minutes
     */
    public long calculateTotalMatchDuration(String matchId) {
        Match match = matchRepository.findById(matchId).orElseThrow(() -> new ResourceNotFoundException("Match not found with id " + matchId));
        if (match.getTiming() == null || match.getTiming().getStartTime() == null || match.getTiming().getEndTime() == null) {
            return 0;
        }
        return java.time.Duration.between(match.getTiming().getStartTime(), match.getTiming().getEndTime()).toMinutes();
    }
    ...
    /**
     * Update the current minute of the match
     */
    public Match updateCurrentMinute(String matchId, int minute) {
        ...
        match.getTiming().setCurrentMinute(minute);
        return matchRepository.save(match);
    }
